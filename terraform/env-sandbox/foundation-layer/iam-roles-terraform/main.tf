...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      AutoCleanup = "true" # Indicates this environment can be cleaned up by automation
    }
  }
}
...
locals {
  environment = "sandbox"
  layers      = ["foundation", "platform", "application", "experiments"]
...
module "foundation_human_role" {
  ...
  session_duration        = 43200 # 12 hours - generous for sandbox experimentation
  permission_boundary_arn = var.permission_boundary_arn
  attach_readonly_policy  = true
...
# Experiments Layer Roles (Sandbox-specific)
module "experiments_human_role" {
  source = "../../../../terraform-modules/terraform-oidc-role"

  role_name               = "terraform-${local.environment}-experiments-human-role"
  environment             = local.environment
  layer                   = "experiments"
  context                 = "human"
  oidc_provider_arn       = var.oidc_provider_arn
...
module "foundation_permission_set" {
  ...
  permission_set_name = "SandboxFoundationAdmin"
...
module "platform_permission_set" {
  ...
  permission_set_name = "SandboxPlatformAdmin"
...
module "application_permission_set" {
  ...
  permission_set_name = "SandboxApplicationAdmin"
...
module "experiments_permission_set" {
  source = "../../../../terraform-modules/permission-set"

  permission_set_name = "SandboxExperimentsAdmin"
  description         = "Sandbox Experiments layer - broad access for experimentation"
  environment         = local.environment
  layer               = "experiments"
  session_duration    = "PT12H"
  ...
  account_assignments = var.experiments_account_assignments
