...
# Production configuration with high availability defaults
# =============================================================================
...
variable "az_count" {
  description = <<-EOT
    ...
    - 3 AZs: Best practice for production, highest availability
    
    Cost impact: ~$32/month per additional NAT Gateway
    
    Production recommendation: 3 AZs for maximum resilience
  EOT
  type        = number
  default     = 3
...
variable "single_nat_gateway" {
  description = <<-EOT
    ...
    Production recommendation: false for high availability
  EOT
  type        = bool
  default     = false
}
...
variable "kubernetes_version" {
  description = <<-EOT
    ...
    Production recommendation: Use tested version from staging
  EOT
  type        = string
  default     = "1.31"
...
variable "cluster_endpoint_public_access" {
  description = <<-EOT
    ...
    Production recommendation: Consider false for maximum security
  EOT
  type        = bool
  default     = true
}
...
variable "cluster_endpoint_public_access_cidrs" {
  description = <<-EOT
    ...
    Production recommendation: Restrict to known IPs/VPN ranges
  EOT
  type        = list(string)
  default     = ["0.0.0.0/0"]
}
...
variable "cluster_enabled_log_types" {
  description = <<-EOT
    ...
    Production recommendation: All types for full visibility
  EOT
  type        = list(string)
...
variable "cloudwatch_log_retention_days" {
  description = <<-EOT
    ...
    - Production typically requires 90+ days for compliance
    
    Production recommendation: 90 days or more based on compliance needs
  EOT
  type        = number
  default     = 90
...
variable "enable_secrets_encryption" {
  description = <<-EOT
    ...
    Production recommendation: Always enable
  EOT
  type        = bool
  default     = true
}
...
variable "enable_cluster_creator_admin_permissions" {
  description = <<-EOT
    ...
    Production consideration: May want to disable after initial setup
  EOT
  type        = bool
  default     = true
}
