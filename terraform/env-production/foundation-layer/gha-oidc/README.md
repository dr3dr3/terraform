...
This Terraform configuration creates the GitHub Actions OIDC provider and IAM role in the **production AWS account**.

## Why This Exists in the Production Account
...
│  │   arn:aws:iam::PROD_ACCOUNT:oidc-provider/             │    │
│  │       token.actions.githubusercontent.com               │    │
│  └─────────────────────────────────────────────────────────┘    │
│                               │                                  │
│                               ▼                                  │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │          IAM Role: github-actions-prod-platform         │    │
...
1. Access to the **production AWS account** with administrative permissions
2. Terraform Cloud workspace configured with credentials for the production account

## Deployment

### Option 1: Via Terraform Cloud (Recommended for ongoing management)

1. Create the Terraform Cloud workspace `production-foundation-gha-oidc`
2. Configure the workspace with AWS credentials for the production account:
   - Set `TFC_AWS_PROVIDER_AUTH = true`
   - Set `TFC_AWS_RUN_ROLE_ARN` to a bootstrap role in the production account
3. Trigger a run

### Option 2: Local Bootstrap (For initial setup)

```bash
# Ensure you're authenticated to the PRODUCTION AWS account
aws sts get-caller-identity
# Should show the production account ID, NOT the management account
...
```bash
terraform output github_actions_prod_platform_role_arn
```

Add this to your GitHub repository secrets:

- **Secret name:** `AWS_ROLE_ARN_PROD_PLATFORM`
...
- [GitHub Actions Workflow](../../../../.github/workflows/terraform-prod-platform-eks.yml)
