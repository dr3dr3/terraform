# Staging Account - Foundation Layer - IAM Roles for Terraform

This workspace creates IAM roles in the **Staging AWS account** that allow Terraform Cloud workspaces to provision resources.

## Overview

This configuration:

1. **Creates an OIDC Provider** in the staging account for Terraform Cloud
2. **Creates Layer-Specific IAM Roles** with appropriate permissions:
   - `terraform-stg-foundation-cicd-role` - For foundation layer workspaces
   - `terraform-stg-platform-cicd-role` - For platform layer workspaces (EKS, VPC)
   - `terraform-stg-applications-cicd-role` - For application layer workspaces
...
│                                                                              │
│  ┌────────────────────┐  ┌────────────────────┐  ┌────────────────────┐     │
│  │ stg-foundation-*   │  │ stg-platform-*     │  │ stg-applications-* │     │
...
│            ▼                       ▼                       ▼                │
│  ┌──────────────────┐  ┌──────────────────┐  ┌──────────────────┐          │
│  │ terraform-stg-   │  │ terraform-stg-   │  │ terraform-stg-   │          │
...
- You need credentials for the staging account BEFORE OIDC is set up

**Bootstrap Options:**

1. **AWS SSO** - Login to staging account and run locally:
...
3. **Cross-Account Role** - Use management account to assume a role in staging (if configured)
...
2. Set environment variables:
   ...
   TFC_AWS_RUN_ROLE_ARN  = "<staging-account-id>:role/terraform-stg-foundation-cicd-role"
   ```
...
|------|-------|-------------|
| `terraform-stg-foundation-cicd-role` | Foundation | IAM roles/policies, OIDC providers |
| `terraform-stg-platform-cicd-role` | Platform | VPC, EKS, KMS, CloudWatch Logs |
| `terraform-stg-applications-cicd-role` | Applications | Lambda, API Gateway, S3, DynamoDB, SNS/SQS |
...
After this workspace is applied, configure other TFC workspaces in the staging project:

### Example: EKS Platform Workspace

```text
TFC_AWS_PROVIDER_AUTH = "true"
TFC_AWS_RUN_ROLE_ARN  = "arn:aws:iam::<staging-account-id>:role/terraform-stg-platform-cicd-role"
```
...
1. **Account Isolation** - Roles exist in staging account, can only access staging resources
