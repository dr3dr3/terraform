...
#
# This configuration creates IAM roles in the STAGING AWS account
# that trust the Terraform Cloud OIDC provider in the MANAGEMENT account.
#
# Architecture:
#   Terraform Cloud → OIDC (management account) → Cross-account assume role → Staging account role
#
# These roles are used by Terraform Cloud workspaces to provision resources
# in the staging AWS account.
...
# Create an OIDC provider in this staging account for Terraform Cloud
...
resource "aws_iam_role" "foundation_cicd" {
  name        = "terraform-stg-foundation-cicd-role"
  description = "Terraform Cloud role for staging foundation layer"
    ...
    Statement = [
      {
        ...
        Condition = {
          ...
          StringLike = {
            "${local.tfc_hostname}:sub" = "organization:${var.tfc_organization}:project:${var.tfc_project_stg}:workspace:*foundation*:run_phase:*"
          }
        }
      }
    ]
...
resource "aws_iam_role_policy" "foundation_permissions" {
  name   = "terraform-stg-foundation-permissions"
...
resource "aws_iam_role" "platform_cicd" {
  name        = "terraform-stg-platform-cicd-role"
  description = "Terraform Cloud role for staging platform layer"
    ...
    Statement = [
      {
        ...
        Condition = {
          ...
          StringLike = {
            "${local.tfc_hostname}:sub" = "organization:${var.tfc_organization}:project:${var.tfc_project_stg}:workspace:*platform*:run_phase:*"
          }
        }
      }
    ]
...
resource "aws_iam_role_policy" "platform_permissions" {
  name   = "terraform-stg-platform-permissions"
...
resource "aws_iam_role" "applications_cicd" {
  name        = "terraform-stg-applications-cicd-role"
  description = "Terraform Cloud role for staging applications layer"
    ...
    Statement = [
      {
        ...
        Condition = {
          ...
          StringLike = {
            "${local.tfc_hostname}:sub" = "organization:${var.tfc_organization}:project:${var.tfc_project_stg}:workspace:*application*:run_phase:*"
          }
        }
      }
    ]
...
resource "aws_iam_role_policy" "applications_permissions" {
  name   = "terraform-stg-applications-permissions"
