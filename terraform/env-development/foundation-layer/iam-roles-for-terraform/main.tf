...
#
# This configuration creates IAM roles in the DEVELOPMENT AWS account (126350206316)
# that trust the Terraform Cloud OIDC provider in the MANAGEMENT account (169506999567).
#
# Architecture:
#   Terraform Cloud → OIDC (management account) → Cross-account assume role → Development account role
#
...
# Create an OIDC provider in this development account for Terraform Cloud
...
resource "aws_iam_role" "foundation_cicd" {
  name        = "terraform-dev-foundation-cicd-role"
  description = "Terraform Cloud role for development foundation layer"
    ...
    Statement = [
      {
        ...
        Condition = {
          ...
          StringLike = {
            "${local.tfc_hostname}:sub" = "organization:${var.tfc_organization}:project:${var.tfc_project_dev}:workspace:*foundation*:run_phase:*"
          }
        }
      }
    ]
...
resource "aws_iam_role_policy" "foundation_permissions" {
  name   = "terraform-dev-foundation-permissions"
...
resource "aws_iam_role" "platform_cicd" {
  name        = "terraform-dev-platform-cicd-role"
  description = "Terraform Cloud role for development platform layer"
    ...
    Statement = [
      {
        ...
        Condition = {
          ...
          StringLike = {
            "${local.tfc_hostname}:sub" = "organization:${var.tfc_organization}:project:${var.tfc_project_dev}:workspace:*platform*:run_phase:*"
          }
        }
      }
    ]
...
resource "aws_iam_role_policy" "platform_permissions" {
  name   = "terraform-dev-platform-permissions"
...
resource "aws_iam_role" "applications_cicd" {
  name        = "terraform-dev-applications-cicd-role"
  description = "Terraform Cloud role for development applications layer"
    ...
    Statement = [
      {
        ...
        Condition = {
          ...
          StringLike = {
            "${local.tfc_hostname}:sub" = "organization:${var.tfc_organization}:project:${var.tfc_project_dev}:workspace:*application*:run_phase:*"
          }
        }
      }
    ]
...
resource "aws_iam_role_policy" "applications_permissions" {
  name   = "terraform-dev-applications-permissions"
